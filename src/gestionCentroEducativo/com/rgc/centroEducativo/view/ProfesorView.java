...
//		gridBagLayout.rowWeights = new double[]{0.0, Double.MIN_VALUE};
		...
		btnPrimero.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				cargarEnPantalla(ControllerProfesor.findFirst());
			}
		});
		...
		btnAnterior.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				cargarEnPantalla(ControllerProfesor.findPrevious(
		...
		btnSiguiente.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				cargarEnPantalla(ControllerProfesor.findNext(
						Integer.parseInt(jtfId.getText())));
			}
		});
		...
		btnUltimo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				cargarEnPantalla(ControllerProfesor.findLast());
			}
		});
		panel.add(btnUltimo);
	...
	private void eliminar() {
	    ...
	    if(opcionElegida == 0) {
		    ...
		    if (ControllerProfesor.eliminar(actualId) != 1) {
		    	JOptionPane.showMessageDialog(null, "Algo ha salido mal");
		    }
		    else {
		    	// Cargo otro registro en pantalla
		    	Profesor pAnterior = ControllerProfesor.findPrevious(actualId);
		    	if (pAnterior != null) {
		    		cargarEnPantalla(pAnterior);
		    	}
		    	else {
		    		Profesor pSiguiente = ControllerProfesor.findNext(actualId);
		    		if (pSiguiente != null) {
	...
	private void guardar() {
		...
		if (p.getId() == 0) {
			int nuevoIdInsertado = ControllerProfesor.insertar(p);
			...
			if (ControllerProfesor.modificar(p) != 1) {
				JOptionPane.showMessageDialog(null, strError);
			}
			else {
				JOptionPane.showMessageDialog(null, "Datos guardados correctamente");
	 ...
	 * @param m
	 */
	private void cargarEnPantalla (Profesor profesor) {
		if (profesor != null) {
			this.jtfId.setText("" + profesor.getId());
			this.jtfNombre.setText(profesor.getNombre());
			this.jtfApellido1.setText(profesor.getApellido1());
			this.jtfApellido2.setText(profesor.getApellido2());
			this.jtfDni.setText(profesor.getDni());
			this.jtfDireccion.setText(profesor.getDireccion());
			this.jtfEmail.setText(profesor.getEmail());
			this.jtfTelefono.setText(profesor.getTelefono());
		}
		
		// Habilito y deshabilito botones de navegación
		if (ControllerProfesor.findPrevious(Integer.parseInt(jtfId.getText())) == null) {
		...
		if (ControllerProfesor.findNext(Integer.parseInt(jtfId.getText())) == null) {
